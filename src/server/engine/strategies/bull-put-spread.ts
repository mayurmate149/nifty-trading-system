/**
 * Bull Put Spread — CREDIT, bullish, 2 legs, defined risk.
 *
 *   SELL near-ATM PE  +  BUY further OTM PE   (net credit)
 *
 * Positive-theta bullish trade. We want bullish structure, decent IV to be
 * paid to sell the PE spread, and a put wall below acting as support.
 */

import type { StrategyRules } from "../strategy-rules/types";
import {
  bollingerExpansionRule,
  dteBetweenRule,
  emaCrossoverRule,
  hasWallRule,
  indicatorTrendRule,
  ivPercentileRule,
  macdBiasRule,
  oiFlowRule,
  pcrBetweenRule,
  rsiBetweenRule,
  spotVsLevelRule,
  superTrendRule,
  vixBetweenRule,
  vwapPositionRule,
} from "../strategy-rules/common-rules";

export const bullPutSpreadRules: StrategyRules = {
  key: "BULL_PUT_SPREAD",
  name: "Bull Put Spread",
  icon: "🟢",
  bias: "CREDIT",
  direction: "BULLISH",
  legs: 2,
  riskProfile: "LIMITED",
  summary: "Sell OTM PE + buy further OTM PE — collect credit with bullish-to-neutral bias.",
  rules: [
    // Trend (weighted but not critical; credit spreads tolerate drift)
    indicatorTrendRule(["trend-up", "range-bound"], 0, 3, false),
    emaCrossoverRule("BULLISH", 3, false),
    superTrendRule("BUY", 2, false),
    // Momentum
    rsiBetweenRule(45, 70, 2, false),
    macdBiasRule("BULLISH", 2, false),
    vwapPositionRule("ABOVE", 2, false),
    // Volatility (rich premium needed)
    ivPercentileRule(30, 85, 3, true),
    vixBetweenRule(11, 26, 1),
    bollingerExpansionRule(1.2, 1),
    // Option chain
    pcrBetweenRule(0.9, 1.7, 2),
    oiFlowRule("put", "BUILDUP", 2),
    hasWallRule("put", "BELOW", 2),
    // Structure
    spotVsLevelRule("support", "ABOVE", 0, 1),
    dteBetweenRule(2, 10, 2, false),
  ],
  exitRules: {
    stopLoss: "Exit at 2× credit received or if short PE strike is breached",
    target: "Close at 50-60% of max profit",
    trailingSL: "After 40% gain, trail SL to breakeven",
    timeExit: "Close 1-2 days before expiry",
  },
};
