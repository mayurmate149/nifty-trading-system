 ...
 */
export function resolveOpenfeedHostFromAccessToken(accessToken: string): string {
  try {
    const parts = accessToken.split(".");
    if (parts.length < 2) return "openfeed.5paisa.com";
    ...
    if (s === "A") return "aopenfeed.5paisa.com";
    if (s === "B") return "bopenfeed.5paisa.com";
...
export function buildXstreamWebSocketUrl(accessToken: string, clientCode: string): string {
